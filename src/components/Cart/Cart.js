          ...
          <Card.Footer style={{ textAlign: "right" }}>
            <Stack direction="horizontal">
              <Card.Text style={{ margin: "5px" }}>
                <s>Original Price: {bag[0]?.[0]?.originalPrice}</s>
              </Card.Text>
              <Card.Text style={{ margin: "5px" }}>
                New Price: {bag[0]?.[0]?.newPrice}
              </Card.Text>
            </Stack>
          </Card.Footer>
        </Card>
        <Alert variant={"danger"}>
          Note: This app is a Capstone Project. Orders will not actually be sent
          to these restaurants, and credit cards will not actually be charged.{" "}
        </Alert>
